...
export default function AdminPaymentsPage() {
  ...
  useEffect(() => {
    adminApi.get('/workers/bookings-all/')
      .then(res => {
        ...
        // Only show bookings that have a price set
  ...
  const unpaid  = bookings.filter(b => b.payment_status === 'unpaid' && b.status === 'completed')

  const totalRevenue    = paid.reduce((s, b) => s + (b.final_price || 0), 0)
  const totalCommission = paid.reduce((s, b) => s + (b.commission_amount || 0), 0)
  const pendingAmount   = unpaid.reduce((s, b) => s + (b.final_price || 0), 0)
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-surface-muted text-sm mt-1">
          {paid.length} paid · {unpaid.length} pending collection
        </p>
      </div>
