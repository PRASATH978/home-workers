  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {[
          { label: 'Pending',     count: counts.pending,     cls: 'badge-orange' },
          { label: 'Accepted',    count: counts.accepted,    cls: 'badge-blue'   },
          { label: 'In Progress', count: counts.in_progress, cls: 'badge-blue'   },
          { label: 'Completed',   count: counts.completed,   cls: 'badge-green'  },
          { label: 'Cancelled',   count: counts.cancelled,   cls: 'badge-red'    },
        ].map(s => (
          <span key={s.label} className={`${s.cls} px-3 py-1.5 text-sm cursor-pointer`}
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-surface-border">
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="text-center py-12 text-surface-muted">
                      No bookings found
                    </td>
                  </tr>
                ) : filtered.map(b => (
                  <tr key={b.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-300">
                      {b.worker?.name || <span className="text-surface-muted italic">Unassigned</span>}
                    </td>
                    <td className="px-4 py-3 text-xs text-surface-muted max-w-36 truncate">
                      {b.address}
                    </td>
