...
export default function AdminServicesPage() {
  ...
  const openEdit = (s) => { setForm({ ...s, base_price: String(s.base_price), sort_order: String(s.sort_order) }); setModal('edit') }

  const handleSave = async () => {
    if (!form.name || !form.slug) return toast.error('Name and slug are required')
    setSaving(true)
    try {
      const payload = { ...form, base_price: parseInt(form.base_price) || 0, sort_order: parseInt(form.sort_order) || 0 }
      if (modal === 'add') await api.post('/services/', payload)
      else await api.patch(`/services/${form.slug}/`, payload)
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-surface-card border border-surface-border rounded-2xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-semibold text-white text-lg">{modal === 'add' ? 'Add Service' : 'Edit Service'}</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="input" placeholder="Plumber" />
              </div>
              <div>
                <label className="text-sm text-slate-400 mb-1.5 block">Slug * (URL-friendly)</label>
                <input value={form.slug} onChange={e => setForm({ ...form, slug: e.target.value.toLowerCase().replace(/\s+/g, '-') })} className="input font-mono" placeholder="plumber" />
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {ICONS.map(i => (
                    <button
                      ...
                      className={`text-xl p-2 rounded-lg border transition-all ${form.icon === i ? 'border-brand-500 bg-brand-500/15' : 'border-surface-border hover:border-slate-500'}`}
                    >
                      {EMOJI[i]}
                    </button>
                  ))}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" value={form.base_price} onChange={e => setForm({ ...form, base_price: e.target.value })} className="input" placeholder="300" />
                </div>
                <div>
                  <label className="text-sm text-slate-400 mb-1.5 block">Sort Order</label>
                  <input type="number" value={form.sort_order} onChange={e => setForm({ ...form, sort_order: e.target.value })} className="input" placeholder="1" />
                </div>
              </div>
              <div>
                <label className="text-sm text-slate-400 mb-1.5 block">Description</label>
                <textarea rows={2} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="input resize-none" placeholder="Brief description…" />
              </div>
            </div>
